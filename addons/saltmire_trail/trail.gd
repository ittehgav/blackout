...
var cfg := {
	"color": Color(1, 1, 1, .5),   # tint of each ghost (alpha = start opacity)
	"interval": 0.02,               # seconds between spawned ghosts
	"lifetime": 0.35,                # seconds each ghost takes to fade out
	"min_distance": 5.0,             # min px moved before a new ghost spawns
	"scale_end": 2,                # ghost scale multiplier at end of life
	"modulate_source": true,         # multiply ghost color by the source modulate
	"z_relative": 2,                # ghost z-index relative to source
}
