...
static func aoe_damage(source:ActiveFighter, hit_scan:Area2D = source.base.hit_scan, hard_value:float=0.0, quiet:bool=false, hit_self:bool=false)->void:
	...
	for area:Area2D in hit_scan.get_overlapping_areas():
		...
		if target != source or hit_self:
			deal_damage(source, target, hard_value, quiet);


static func aoe_status(source:ActiveFighter, status:Status=source.base.skill.status, \
						hit_scan:Area2D=source.base.hit_scan, hard_value:float = 0, hit_self:bool=false)->void:
...
static func aoe_knockback(source:ActiveFighter, hit_scan:Area2D = source.base.hit_scan,\
					strength:int = source.base.skill.knockback_strength, hit_self:bool=false)->void:
	var hurtboxes:Array[Area2D] = hit_scan.get_overlapping_areas();
	for area:Area2D in hurtboxes:
		assert(area is HurtBox);
		var target:CombatEntity = area.source
