...
@onready var icon_anchor = icon.position;
...
func set_tide_bar()->void:
	var team_1_power:=0;
	var team_2_power:=0;
	for key in team_1_unit_values.keys():
		team_1_power += team_1_unit_values[key]
	for key in team_2_unit_values.keys():
	...
	var player_fighter_level = Entities.player.combat_level;
	team_1_unit_values[Entities.in_fight_player] = player_fighter_level;
	max_value += player_fighter_level
	value += player_fighter_level
...
func on_unit_death(_killer:ActiveFighter, victim:ActiveFighter)->void:
	...
	if victim.ally_team == Entities.arena.team_1:
		...
		$sfx.play()
		value -= team_1_unit_values[victim]
	else:
...
func alert_shake_tween():
	const shake_range = 5;
	var x_roll = randi_range(icon_anchor.x - shake_range, icon_anchor.x + shake_range)
	var y_roll = randi_range(icon_anchor.y - shake_range, icon_anchor.y + shake_range);
...
func enable_icon_tween()->void:
	...
	## make smaller fights only end when a party is wiped out?
	if enemy_value <= 0:
