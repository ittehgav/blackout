...
func assign_fighter(fighter:ActiveFighter)->void:
	...
	fighter.ally_team = self
	fighter.enemy_team = enemy_team;
	
	fighter.death.connect(on_fighter_death.bind(fighter))

	fighter.hurtbox.set_collision_layer_value(team_n, true);
	if not fighter is PlayerFighter:
...
func unassign_fighter(fighter:ActiveFighter)->void:
	...
	fighter.death.disconnect(on_fighter_death);
