...
func equip_command()->void:
	...
	if item is Weapon:
		if not Entities.player.alternative_weapon:
			...
			equip_weapon_command(Input.is_key_pressed(KEY_ALT));
		...
		equip_artifice_command()



func equip_weapon_command(alt:bool=false)->void:
	## BUG: sometimes when equipping weapon it just crashes?
	## seems to have to do with swapping?
...
func equip_artifice_command()->void:
	display.artifice_slot_selector.show_options(item)
...
func equip_accessory_on_player()->void:
	...
	if not Entities.player.equipped_accessory_1:
		...
		if not display.has_room(Entities.player.equipped_accessory_2, item):
			display.invalid_move.emit("NOT ENOUGH ROOM")
			return;
		just_unequipped = Entities.player.equip_accessory(item, 2);
		
	if just_unequipped:
		## room confirmed before equipping
		display.throw_in_inventory(just_unequipped)
	display.remove_mirror(self, false);
...
func use_consumable_command()->void:
	match item.use_type:
		"instant":
			...
			item.use();
			Entities.player_sheet.consumable_feedback(item);

			display.remove_mirror(self)
			
		"unit":
			if item.tag_target != -1:
				if not len(Entities.player.roster.units.filter(item.filter_valid_target)):
		...
		"item":
			...
			if item.item_target != -1:
				match item.item_target:
					...
					"artifice":
						item_pool = Entities.player.inventory.artifices;
			else:
...
func _on_mouse_entered() -> void:
	display.hover_sfx.play()
	
	if not shaking:
		var tween:Tween = Tweens.mouseover_shake(self, PI/64, .1);
		tween.tween_callback(func()->void:shaking=false)
...
func refresh()->void:
	...
	artifice_equipped_label.hide()
	if item is Artifice and item.unique_name in Entities.player.equipped_artifices_names.values():
		var key:int = Entities.player.equipped_artifices_names.find_key(item.unique_name);
		artifice_equipped_label.text = str(key);
		artifice_equipped_label.show()
		artifice_equipped_label.modulate = self_modulate
