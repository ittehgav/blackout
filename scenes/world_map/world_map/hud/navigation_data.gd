...
var current_target:Location

@warning_ignore("shadowed_global_identifier")
@export var sign:LocationSign
...
func show_location(target:Location)->void:
	name_label.text = target.unique_name
	set_travel_data(target);
	Tweens.ui_fade_in(self);

func set_travel_data(target:Location)->void:
	current_target = target
	sign.load_location(target)
	...
	var arrival_hour:int = world_map.current_hour + travel_hours;
	var arrival_minute:int = world_map.current_minute + travel_minutes;
...
func _on_button_pressed() -> void:
	clear();
	Entities.player_party.move_to_location(current_target)
