...
func intimidate_odds(target:Leader)->float:
	var combined_level:int = leader.level * 2;
...
func convince_odds(target:Leader)->float:
	var level_gap:int = leader.level - target.leader.level;

	
	var odds: = .8;
	odds += .1 * leader.discipline.charisma
	var per_level_decay: = .8;
	per_level_decay += .025 * leader.discipline.charisma;
	for i:int in level_gap:
		odds *= per_level_decay
	return odds
	
func roll_intimidate(target:Leader)->bool:
...
func roll_convince(target:Leader)->bool:
	var roll: = randf_range(0, 1);
...
func _on_settlement_visited(_settlement: Settlement) -> void:
	stopped_moving.emit();
