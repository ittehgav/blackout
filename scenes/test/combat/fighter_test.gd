...
func log_dmg_blocked(signal_source:ActiveFighter, quiet:bool)->void:
	print(signal_source.base.name + "damage blocked, quiet = ", str(quiet))

func log_dmg_taken(damage:float, source:ActiveFighter, quiet:bool, signal_source:ActiveFighter)->void:
	print(signal_source.base.name +" "+ str(damage) +" damage taken from " + source.name, " quiet = ", str(quiet))
...
func log_shield_gained(source:ActiveFighter, value:float, quiet:bool,  signal_source:ActiveFighter)->void:
	print(signal_source.base.name + " " +str(value) + " shield gained from " + source.name, " quiet = " + str(quiet));
