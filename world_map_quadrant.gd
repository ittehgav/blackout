...
func load_fog(data:Array)->void:
	## RLE ARRAY = [cell type, amount]
	## -1 = off-sight = nothing (current seen area is refreshed in-session)
	## 2 = off sight = Vector2(2, 0)
	## 0 = fog = Vector2(0, 0)
	var x:int = -1;
	var y:int = 0;
	
	var x_max:int = get_parent().quarter_tile_map_size.x;
	for rle:Array in data:
		var type:int = rle[0];
		var cell_type:Vector2i;
		if type == 0:
			cell_type = Vector2i.ZERO;
		elif type == 2 or type == -1:
			cell_type = Vector2i(2, 0)
		...
		for i:int in rle[1]:
			...
			fog_tile_map.set_cell(Vector2(x, y), 0, cell_type);
...
func _ready()->void:
	...
	if get_parent().new_game:
		## otherwise is already generated by the time this runs
		for x in map_size.x:
			for y in map_size.y:
